...
case class OuroborosBlock(override val parentId: BlockId,
  ...
  override type M = OuroborosBlock

  override lazy val modifierTypeId: Byte = OuroborosBlock.ModifierTypeId

  override lazy val transactions: Option[Seq[BifrostTransaction]] = Some(txs)

  override lazy val serializer = OuroborosBlockCompanion
...
object OuroborosBlock {
  ...
  def create(parentId: BlockId,
             ...
             inflation: Long,
             version: Version): OuroborosBlock = {
    assert(box.proposition.pubKeyBytes sameElements privateKey.publicKeyBytes)

    val unsigned = OuroborosBlock(parentId, timestamp, box, Signature25519(Array.empty), txs, inflation, version)
...
object OuroborosBlockCompanion extends Serializer[OuroborosBlock] {

  def commonMessage(block: OuroborosBlock): Array[Byte] = {
  ...
  def commonMessage2xAndBefore(block: OuroborosBlock): Array[Byte] = {
    val numTx = Ints.toByteArray(block.txs.length)
    val generatorBoxBytes = BifrostBoxSerializer.toBytes(block.forgerBox)
  ...
  def messageToSign(block: OuroborosBlock): Array[Byte] = {
  ...
  override def toBytes(block: OuroborosBlock): Array[Byte] = {
    block.version match {
      case 0 =>
  ...
  override def parseBytes(bytes: Array[ModifierTypeId]): Try[OuroborosBlock] = Try {
    ...
    OuroborosBlock(parentId, timestamp, generatorBox, signature, tx, Longs.fromByteArray(inflation), version)
  }


  def parseBytes2xAndBefore(bytes: Array[ModifierTypeId]): Try[OuroborosBlock] = Try {
    val parentId = bytes.slice(0, Block.BlockIdLength)
    val Array(timestamp: Long, generatorBoxLen: Long) = (0 until 2).map {
      i => Longs.fromByteArray(bytes.slice(Block.BlockIdLength + i * Longs.BYTES, Block.BlockIdLength + (i + 1) * Longs.BYTES))
    }.toArray
    ...
    OuroborosBlock(parentId, timestamp, generatorBox, signature, tx, protocolVersion = version)
  }
}